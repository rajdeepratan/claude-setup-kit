...
1. **Pre-plan checks** — run two checks before drafting the plan: (a) **graph freshness** if `graphify-out/graph.json` is present (`claude-setup-graph.md` Runtime section); (b) **`.claude/` coverage** for new-domain detection (`claude-setup-coverage.md`). Both auto-skipped on `/quick` and `/code` trivial. Then, if superpowers is installed, invoke `superpowers:writing-plans` to produce a structured plan.
